...
export function generateCompetenceMatrixPdf(options: CompetenceMatrixOptions): jsPDF {
  const { personnel, projectName, companyName, location, activePersonnelCount } = options;
  ...
  // ── Header drawing function (called on every page after table render) ──
  ...
  const drawHeader = () => {
    ...
    doc.text(`Total Personnel: ${personnel.length}`, rightCol, y);
    y += 4.5;

    if (location) {
      doc.text(`Location: ${location}`, leftCol, y);
    }
    if (activePersonnelCount !== undefined) {
      doc.text(`Active Personnel: ${activePersonnelCount}`, rightCol, y);
    }
    y += 4.5;

    doc.text(`Expiry warning: ${EXPIRY_WARNING_DAYS} days`, leftCol, y);
    y += 3;
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      ...
      if (colIdx < 3) return; // skip Name/Role/Type columns
